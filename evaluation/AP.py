...
def line_matching_by_CD(inst_pred_lines, inst_pred_confidence, inst_label_lines, threshold):
    ...
    for i, key_pred in enumerate(inst_pred_lines_keys):
        for j, key_label in enumerate(inst_label_lines_keys):
            CD[i, j] = chamfer_distance(inst_pred_lines[key_pred][None], inst_label_lines[key_label][None],
                                        bidirectional=True, threshold=threshold)
...
def instance_mask_AP(AP_matrix, AP_count_matrix, inst_pred_mask, inst_label_mask, scale_x, scale_y, confidence,
                     thresholds, sampled_recalls):
    # inst_pred: N, C, H, W
    # inst_label: N, C, H, W
    ...
    for n in range(N):
        for c in range(C):
            ...
            if len(inst_pred_lines) == 0 and len(inst_label_lines) == 0:
                continue
            AP_matrix[c] += single_instance_line_AP(inst_pred_lines, confidence[n], inst_label_lines, thresholds,
...
def single_instance_line_AP(inst_pred_lines, inst_pred_confidence, inst_label_lines, thresholds, sampled_recalls):
    ...
    for t in range(num_thres):
        ...
        precision, recall = get_precision_recall_curve_by_confidence(matching_list, len(inst_label_lines),
                                                                     thresholds[t])
